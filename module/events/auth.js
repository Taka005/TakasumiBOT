async function auth(interaction){
  if(interaction.customId.startsWith("auth_")){
    ...
    if(interaction.guild.member.roles.cache.has(role_id)) return interaction.reply({content: "既に役職を付与済みです",ephemeral: true});
    const role = interaction.guild.roles.cache(roll);
    interaction.guild.member.roles.add(role)
